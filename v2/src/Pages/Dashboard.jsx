...
export default function Dashboard() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full p-5 gap-6">
      ...
      <div className="flex items-center gap-3 px-1 shrink-0">
        ...
        <div>
          <p className="text-sm font-semibold text-white">{td.title}</p>
          <p className="text-xs text-gray-500">{td.adminPanel}</p>
        </div>
      </div>
      ...
      <div className="shrink-0 px-3 py-2 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center gap-2">
        ...
        <span className="text-indigo-300 text-xs font-medium">{td.portfolioManager}</span>
      </div>

      {/* Nav */}
      <nav className="flex flex-col gap-1 flex-1 min-h-0">
        <p className="text-[10px] text-gray-600 uppercase tracking-widest px-3 mb-2 shrink-0">{td.menu}</p>
  ...
  return (
    <div className="flex text-white" style={{ height: '100dvh' }}>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-30 w-60 flex flex-col border-r border-white/8 bg-canvas-alt backdrop-blur-xl transition-transform duration-300 lg:hidden ${
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0">
        ...
        <div className="lg:hidden flex items-center gap-3 px-4 py-3 border-b border-white/8 bg-white/3 backdrop-blur-xl shrink-0">
          ...
          <span className="text-sm font-medium text-white">{td.title}</span>
