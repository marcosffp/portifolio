...
export default function Comments() {
  ...
  const remove = async (id) => {
    if (!confirm(td.confirmDeleteComment)) return;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start sm:items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-1 p-1 rounded-xl bg-white/5 border border-white/10">
          {[
            { value: "all", label: td.all, count: comments.length },
            { value: "pinned", label: td.pinned, count: pinnedCount },
          ].map((tab) => (
            <button
              key={tab.value}
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: td.total, value: comments.length, color: "text-indigo-400" },
          { label: td.pinned, value: pinnedCount, color: "text-purple-400" },
          { label: td.unpinned, value: comments.length - pinnedCount, color: "text-blue-400" },
        ].map((stat) => (
          <Card key={stat.label}>
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {comment.is_pinned && (
                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-indigo-500/15 border border-indigo-500/25 text-indigo-300 text-xs">
                          <Pin className="w-2.5 h-2.5" /> {td.pinnedBadge}
                        </span>
                      )}
