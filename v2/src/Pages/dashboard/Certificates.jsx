...
export default function Certificates() {
  ...
  const deleteCert = async (id) => {
    if (!confirm(td.deleteCert)) return
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-white">{td.certificates}</h1>
          <p className="text-gray-500 text-xs">
            {loading ? td.loading : `${certs.length} ${td.certificates.toLowerCase()} total`}
          </p>
        </div>
      </div>
      ...
      <Card>
        <div className="p-5 sm:p-6 space-y-4">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            <Plus className="w-4 h-4 text-indigo-400" /> {td.uploadCert}
          </h2>
          ...
          >
            {preview ? (
              ...
              <div className="text-center space-y-2 p-6">
                ...
                <p className="text-sm text-gray-300">{td.dragDrop}</p>
                <p className="text-xs text-gray-600">{td.certFormats}</p>
              </div>
            )}
      ...
      {loading ? (
        ...
        <Card>
          <div className="p-16 text-center">
            ...
            <p className="text-gray-500 text-sm">{td.noCerts}</p>
          </div>
        </Card>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
