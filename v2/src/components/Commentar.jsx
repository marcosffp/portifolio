...
const Comment = memo(({ comment, formatDate, index, isPinned = false, pinnedLabel, adminLabel }) => (
    <div
...
const CommentForm = memo(({ onSubmit, isSubmitting, error, tc }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2" data-aos="fade-up" data-aos-duration="1200">
                <label className="block text-sm font-medium text-white">
                    {tc.message} <span className="text-red-400">*</span>
                </label>
                <textarea
                    ref={textareaRef}
                    value={newComment}
            ...
            <div className="space-y-2" data-aos="fade-up" data-aos-duration="1400">
                <label className="block text-sm font-medium text-white">
                    {tc.profilePhoto} <span className="text-gray-400">{tc.optional}</span>
                </label>
...
const Komentar = () => {
    ...
    const formatDate = useCallback((timestamp) => {
        ...
        if (diffMinutes < 1) return tc.justNow;
        if (diffMinutes < 60) return `${diffMinutes}${tc.minutesAgo}`;
        if (diffHours < 24) return `${diffHours}${tc.hoursAgo}`;
        if (diffDays < 7) return `${diffDays}${tc.daysAgo}`;

        return new Intl.DateTimeFormat(tc.locale, {
            year: 'numeric',
            month: 'short',
            day: 'numeric'
        }).format(date);
    ...
    return (
        <div className="w-full bg-gradient-to-b from-white/10 to-white/5 rounded-2xl  backdrop-blur-xl shadow-xl" data-aos="fade-up" data-aos-duration="1000">
            ...
            <div className="p-6 space-y-6">
                ...
                <div>
                    <CommentForm onSubmit={handleCommentSubmit} isSubmitting={isSubmitting} error={error} tc={tc} />
                </div>
